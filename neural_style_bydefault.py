# command for running the code of masking object : AI_art
'''
python neural_style_bydefault.py --content_img "24.jpg" \
                                --style_imgs "8.jpg" \
                               --max_size 1000 \
                              --max_iterations 200 \
                              --style_mask \
                              --style_mask_imgs "24_mask.jpg" 
                              --verbose
'''
...
'''
def stylize(content_img, style_imgs, init_img, content_img_val ,style_img_val, frame=None):
  with tf.device(args.device), tf.Session() as sess:
    ...
    if args.video:
      ...
      write_image_output(output_img, content_img, style_imgs, init_img, content_img_val,style_img_val)
...
def write_image_output(output_img, content_img, style_imgs, init_img, content_img_val,style_img_val):
  out_dir = os.path.join(args.img_output_dir)
  maybe_make_directory(out_dir)
  # img_path = os.path.join(out_dir, args.img_name+'.png')
  img_path = os.path.join(out_dir, f'{content_img_val}_{style_img_val}'+'.png')
...
def render_single_image(content_img_val,style_img_val):
  ...
  with tf.Graph().as_default():
    ...
    tick = time.time()
    stylize(content_img, style_imgs, init_img,content_img_val,style_img_val)
    tock = time.time()
...
def main():
  ...
  output_dir = args.img_output_dir
  style_img = args.style_imgs


  content_img_val = content_img.split('.')[0]
  style_img_val = str(style_img[0].split('.')[0])

  render_single_image(content_img_val,style_img_val)
